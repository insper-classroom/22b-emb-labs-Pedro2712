...
#define BUT1_PIO_PIN_MASK (1u << BUT1_PIO_IDX) // esse já está pronto.
...
void but_callback(void) {
    ...
    uint32_t but = 1;
    xQueueGiveFromISR(xQueueBut, (void *)&but, 10);
}
...
static void task_led(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (xQueueReceive(xQueueLedFreq, &msg, delay_ms)) {
...
static void task_but(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (xQueueReceive(xQueueBut, &but, 10)) {
            if (but){
                delayTicks-= 100;
...
int main(void) {
    ...
    if (xQueueLedFreq == NULL || xQueueBut == NULL)
