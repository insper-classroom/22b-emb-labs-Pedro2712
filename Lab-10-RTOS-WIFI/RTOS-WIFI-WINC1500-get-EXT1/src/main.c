...
SemaphoreHandle_t xSemaphorePOST;
...
void post_callback(uint8_t u8MsgType, void *pvMsg){
    printf("Entrou\n");
    if (strcmp(POSTDATA, "LED=0") == 0){
        POSTDATA = "LED=1";
    } else {
        POSTDATA = "LED=0";
    }
...
void io_init(void) {
    ...
    // com prioridade 0 (quanto mais pr�ximo de 0 maior)
    NVIC_EnableIRQ(BUT_PIO_ID);
    NVIC_SetPriority(BUT_PIO_ID, 4); // Prioridade 0
}
...
static void get(uint8_t g_sendBuffer) {
    printf("STATE: GET \n");
  ...
  while(1){
    ...
    switch(state){
      ...
      if(xQueueReceive(xQueueMsg, &p_recvMsg, 5000) == pdTRUE){
        printf(STRING_LINE);
        char *ret;
        ret = strstr(p_recvMsg->pu8Buffer, "1");
        if(ret != NULL) {
...
int main(void)
{
  ...
  xSemaphorePOST = xSemaphoreCreateBinary();

  // verifica se semáforo foi criado corretamente
  if (xSemaphorePOST == NULL)
      printf("falha em criar o semaforooooooooooooooooo \n");
